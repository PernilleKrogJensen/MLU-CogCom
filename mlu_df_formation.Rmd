...
My notes: I can just write data instead of the full working reposatory because I am already in the right working reposatory where the folder "data" already is. 

```{R}
Data_list <- list.files(path= "data", full.names = T)
```
...
```{R}
sarah_data <- map_df(Data_list,function(x) read.CLAN.file(x))
```
...
```{R}
simple_sarah_data <- select(sarah_data, "Speaker", "Gloss", "Date")
```
...
My notes: If I want to make sure that it ends up in the right folder, i can write the full path before "simple sarah data.csv"
```{R}
?write.csv

write.csv(simple_sarah_data, "simple sarah data.csv")
...
```{R}
sarah_data <- read.csv("simple sarah data.csv", header = T)
...
My notes: So I need to change speaker to factor, now it is in chr. Gloss is alreadt in chr but I als need to change date to date
```{R}

str(simple_sarah_data)
...
simple_sarah_data$Speaker <- as.factor(simple_sarah_data$Speaker)

simple_sarah_data$Date <- anytime::anydate(simple_sarah_data$Date)
...
mother_child_df<-filter(simple_sarah_data, simple_sarah_data$Speaker == "MOT" | simple_sarah_data$Speaker == "CHI") 
...
mother_child_df$clean = str_replace_all(mother_child_df$Gloss, "[:punct:]", "")
...
mother_child_df$MLU = stri_count_words(mother_child_df$clean)
...
new_mother_child_df <- group_by(mother_child_df, Speaker, Date)
...
mlu_df <- summarise(new_mother_child_df, words = sum(stri_count_words(clean)), 
                    utterances = n(), 
                    mlu = sum(stri_count_words(clean)) / n())
